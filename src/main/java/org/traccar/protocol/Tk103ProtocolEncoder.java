...
public class Tk103ProtocolEncoder extends StringProtocolEncoder {
    ...
    private String getTk103Id(long deviceId) {
        return getCacheManager().getObject(Device.class, deviceId).getTk103Id();
    }
    ...
    protected String formatCommand(Command command, String format, ValueFormatter valueFormatter, String... keys) {
        ...
        for (int i = 0; i < keys.length; i++) {
            ...
            } else if (keys[i].equals(Command.KEY_TK103_ID)) {
                value = getTk103Id(command.getDeviceId());
            } else {
    ...
    protected Object encodeCommand(Command command) {
        ...
        if (alternative || forceAlternative) {
            ...
            switch (command.getType()) {
                case Command.TYPE_CUSTOM:
                    return formatCommand(command, "(%s%s)", Command.KEY_TK103_ID, Command.KEY_DATA);
                case Command.TYPE_GET_VERSION:
                    return formatCommand(command, "(%sAP07)", Command.KEY_TK103_ID);
                case Command.TYPE_REBOOT_DEVICE:
                    return formatCommand(command, "(%sAT00)", Command.KEY_TK103_ID);
                case Command.TYPE_SET_ODOMETER:
                    return formatCommand(command, "(%sAX01)", Command.KEY_TK103_ID);
                case Command.TYPE_POSITION_SINGLE:
                    return formatCommand(command, "(%sAP00)", Command.KEY_TK103_ID);
                case Command.TYPE_POSITION_PERIODIC:
                    String frequency = String.format("%04X", command.getInteger(Command.KEY_FREQUENCY));
                    return formatCommand(command, "(%sAR00" + frequency + "0000)", Command.KEY_TK103_ID);
                case Command.TYPE_POSITION_STOP:
                    return formatCommand(command, "(%sAR0000000000)", Command.KEY_TK103_ID);
                case Command.TYPE_ENGINE_STOP:
                    return formatCommand(command, "(%sAV010)", Command.KEY_TK103_ID);
                case Command.TYPE_ENGINE_RESUME:
                    return formatCommand(command, "(%sAV011)", Command.KEY_TK103_ID);
                case Command.TYPE_OUTPUT_CONTROL:
                    return formatCommand(command, "(%sAV00%s)", Command.KEY_TK103_ID, Command.KEY_DATA);
                default:
                    return null;
            }
        }
